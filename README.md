...
> - DSH **0.1.1-rc.2**（npm 最新）：装**旧版** `'@dsh-external/dsh-minigames@github:dsh-external/dsh-minigames#v0.3.5'`
...
```sh
# 方式一：git 依赖固定 tag（公开镜像，推荐；也可用 github:dsh-external/dsh-minigames）
dsh plugin --profile web add '@dsh-external/dsh-minigames@github:dsh-external/dsh-minigames#v0.3.23'
...
### 2026-09-24 · v0.3.23 — 声明支持 dsh-v0.1.7-rc.1

- **声明**：支持 dsh-v0.1.7-rc.1（舰队扫检零错误零崩溃，零适配改动）；typecheck/210 单测/构建全绿
...
- **验证**：typecheck/build/203 单测全绿；实机复现原步骤（缩小→拖至右缘→放大）确认按钮自动回到可视区

### 2026-09-15 · v0.3.20 — 声明支持 dsh-v0.1.6-alpha.1

声明支持 dsh-v0.1.6-alpha.1（npm 已发布，钉版本实机验证；client 插件面零代码差异，typecheck/203 单测全绿，实机加载正常）。安装命令统一更新为 `#v0.3.23`。
...
> 1. 执行 `dsh plugin --profile web add '@dsh-external/dsh-minigames@github:dsh-external/dsh-minigames#v0.3.23'`（首次可能被 pnpm 11 拦截 node-pty 构建脚本而失败）
> 2. 在 `~/.dsh/profiles/web` 下执行 `pnpm approve-builds --all`（放行构建脚本）
> 3. 再执行一次第 1 步的安装命令
> 4. 完成后提醒我硬刷新浏览器（Ctrl/Cmd+Shift+R）
> 遇到报错先查 https://github.com/dsh-external/dsh-minigames README 的常见问题/已知限制。
