...
- **明暗主题**：标题栏可切换白天 / 夜间外观，覆盖窗口、游戏库、按钮、暂停与确认界面和启动器；选择会记住，切换不重建本局。
- **游戏库**：按益智、棋盘、街机、反应分类，支持名称与玩法搜索；有历史成绩时展示最佳分数。
- **浮动窗口**：拖动标题栏移动，使用右上角菜单选择浮动、靠左或靠右；拖动边缘调整宽度，
  也可聚焦调整手柄后用左右方向键调整。小屏幕会限制窗口尺寸和位置，避免超出视口。
- **本局进度**：隐藏窗口或返回游戏库会暂停并保留当前实例，重新打开后点“继续游戏”；
  切换游戏和重新开始均先确认。刷新页面后回到游戏库，最高分和窗口位置仍保留，本局不跨刷新保存。
- **操作**：选择游戏后先看说明，再点“开始游戏”。暂停时显示遮罩；“玩法”展开完整按键说明，
  工具栏与 P / R 快捷键共用暂停和重开流程。焦点离开游戏区域或切换浏览器标签时自动暂停，需手动继续。
...
- **验证**：typecheck/build/203 单测全绿；热挂载实机验证（青色「后台运行中」状态 + Token 用量面板为本轮新增功能，见下方更新记录）
...
1. 打开 DSH Web UI，点击浮动的游戏手柄按钮进入游戏库。
2. 按分类或搜索找到游戏，点击后查看说明，再点“开始游戏”。
3. 用“暂停”按钮或 `P` 暂停/继续；用“重新开始”或 `R` 打开重开确认。
4. 点“游戏库”返回列表；“继续”入口保留本局。选另一款游戏时确认后才结束旧局。
5. 右上角 `×` 隐藏窗口，当前游戏暂停；重新打开后手动继续。
6. 右上角菜单可直接选择窗口位置；拖动标题栏或窗口边缘可调整布局。

数独可点底部数字键填入或清除，切换难度时先确认，普通重开保留当前难度；扫雷可切换“翻开 / 标旗”后点击格子，无需依赖右键。记忆翻牌只在通关后记录成绩，旧版未通关过程分不再作为最佳纪录（原数据保留）。
...
`create()` 新实例；隐藏窗口、返回游戏库、失焦或用户点暂停时调用 `pause()`，继续时调用 `resume()`。`P` 和 `R` 通过 `options.onPauseRequest` / `options.onRestartRequest` 交由面板统一处理；重开确认后默认销毁并重新创建实例；需要保留难度等局部设置时可实现 `restart()`，或向 `onRestartRequest` 传入确认后执行的动作，取消不会执行。
...
- 键盘控制依赖游戏区域焦点；点击聊天输入框后游戏自动暂停，返回后点“继续游戏”。
