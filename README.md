...
- 恐龙雨天/昼夜为纯视觉氛围，不影响判定。
`v0.3.19`（默认） | `dsh-v0.1.2-alpha.1`~`alpha.5`、`rc.1`、`0.1.3-alpha.1`~`0.1.5-rc.2` | 声明支持 0.1.5-rc.1~rc.2（npm 已发布，钉版本实机验证；rc.1 为 0.1.5 系列首个候选版本，client 插件面零代码差异；typecheck/build/203 单测全绿，启动清单确认加载；rc.2 兼容性复验通过：rc.2 实机宿主（tag fb2c4b9e）加载确认，无需代码改动） |
